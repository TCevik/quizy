...
class DictationQuiz extends BaseQuiz {
    constructor() {
        super('dictation', 'dict-overlay', 'sp-overlay');
        this.answered = false;
    ...
    renderLayout() {
        ...
        const mainContent = `
            <div class="sp-container" style="position: relative;">
                <div class="sp-header">
                    <span class="sp-title">${escapeHtml(state.currentSet.title || 'Dictaat')}</span>
                    <div style="display: flex; gap: 8px; align-items: center; position: relative;">
                        <button class="btn-close-flashcards" id="dict-info-btn" title="Toetsenbord sneltoetsen" style="transform: none;">
                            <span class="material-symbols-rounded">info</span>
                        </button>
                        <button class="btn-close-flashcards" id="dict-settings-btn" title="Instellingen" style="transform: none;">
                            <span class="material-symbols-rounded">settings</span>
                        </button>
                        <button class="btn-close-flashcards" id="dict-close">
                            <span class="material-symbols-rounded">close</span>
                        </button>
                        <quizy-settings-panel id="dict-settings-panel" mode="dictation"></quizy-settings-panel>
                    </div>
                </div>
                ...
                <div style="width: 100%;">
                    <form id="dict-form" class="sp-input-form" autocomplete="off" onsubmit="return false;">
                        ...
                        <div class="sp-action-area">
                            ...
                            <button type="button" class="btn-sp-action secondary" id="dict-btn-skip">
                                Overslaan
                            </button>
                            <button type="submit" class="btn-sp-action primary" id="dict-btn-submit" style="background: var(--primary); color: white; border-color: var(--primary);">
                                Controleren
                            </button>
                        </div>
                    </form>
                ...
                <div class="progress-container">
                    <span class="progress-text" id="dict-progress-text">Geleerd: 0 van ${totalUniqueCards} kaarten</span>
    ...
    setupElements() {
        this.questionLabelEl = this.overlay.querySelector('#dict-question-label');
        this.userInputEl = this.overlay.querySelector('#dict-user-input');
        this.feedbackContainer = this.overlay.querySelector('#dict-feedback-container');
        this.submitBtn = this.overlay.querySelector('#dict-btn-submit');
        this.skipBtn = this.overlay.querySelector('#dict-btn-skip');
        this.spForm = this.overlay.querySelector('#dict-form');
        this.closeBtn = this.overlay.querySelector('#dict-close');
        this.progressTextEl = this.overlay.querySelector('#dict-progress-text');
        this.progressFillEl = this.overlay.querySelector('#dict-progress-fill');
        this.replayBtn = this.overlay.querySelector('#dict-replay-btn');
        this.hintBtn = this.overlay.querySelector('#dict-hint-btn');
        this.hintContainer = this.overlay.querySelector('#dict-hint-container');
        this.settingsBtn = this.overlay.querySelector('#dict-settings-btn');
        this.settingsPanel = this.overlay.querySelector('#dict-settings-panel');
        this.confirmModal = this.overlay.querySelector('#dict-confirm-modal');
        this.infoBtn = this.overlay.querySelector('#dict-info-btn');
        this.keybindsModal = this.overlay.querySelector('#dict-keybinds-modal');
        this.overrideBtn = this.overlay.querySelector('#dict-btn-override');
    }
    ...
    addEventListeners() {
        if (this.infoBtn && this.keybindsModal) {
            this.infoBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                this.keybindsModal.open('dictation');
            });
        }
        ...
        this.keydownHandler = (e) => {
            ...
            if (e.code === 'ControlLeft' || e.code === 'ControlRight') {
                e.preventDefault();
                this.speakCurrentQuestion();
            } else if (this.answered && (e.code === 'Enter' || e.code === 'Space')) {
                e.preventDefault();
    ...
    updateQuestion() {
        ...
        if (this.settings.swapSides) {
            if (state.currentSet.mode === 'talen') {
                this.questionLabelEl.textContent = `Typ in het ${state.currentSet.lang2 || 'Definitie'}`;
            } else {
                this.questionLabelEl.textContent = 'Typ de definitie';
            }
        } else {
            if (state.currentSet.mode === 'talen') {
                this.questionLabelEl.textContent = `Typ in het ${state.currentSet.lang1 || 'Term'}`;
            } else {
                this.questionLabelEl.textContent = 'Typ de term';
            }
        }
        ...
        if (this.learnedCardKeys.size === this.originalCards.length) {
            this.overlay.innerHTML = `
                <div class="sp-container" style="text-align: center;">
                    ...
                    <p style="color: var(--text-muted); margin-bottom: 30px;">Je hebt alle dictaat-opdrachten in deze set voltooid!</p>
                    <div style="display: flex; gap: 16px; justify-content: center;">
                        <button class="btn-control" id="dict-restart" style="background: var(--primary); color: white; border-color: var(--primary);">Opnieuw oefenen</button>
                        <button class="btn-control" id="dict-finish-close" style="background: rgba(255,255,255,0.1);">Sluiten</button>
                    </div>
                </div>
            `;
            
            this.overlay.querySelector('#dict-restart').addEventListener('click', () => {
            ...
            this.overlay.querySelector('#dict-finish-close').addEventListener('click', () => {
                this.closeOverlay();
            });
            return true;
        }
    ...
    handleFormSubmit() {
        ...
        const card = this.activeQueue[this.currentIndex];
        const correctAnswer = this.settings.swapSides ? card.definition : card.term;
                ...
                this.feedbackContainer.innerHTML = `
                    <div class="sp-feedback-card correct">
                        ...
                        <div class="feedback-text-container">
                            ...
                            <span class="feedback-desc">Goed gehoord en gespeld.</span>
                        </div>
                    </div>
                `;
            }
...
const dictationInstance = new DictationQuiz();

document.addEventListener('DOMContentLoaded', () => {
    document.addEventListener('click', (e) => {
        const btnDictation = e.target.closest('#btn-dictation');
        if (btnDictation) {
            e.preventDefault();
            dictationInstance.open();
        }
    });
});
