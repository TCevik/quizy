...
class QuizyHeader extends HTMLElement {
    ...
    getLogoHTML(sub) {
        if (sub && sub !== 'none') {
            return `Quizy <span style="font-size: 0.55em; vertical-align: middle; color: var(--primary); font-weight: 800; margin-left: 6px; border: 1.5px solid var(--primary); padding: 1px 6px; border-radius: 6px; letter-spacing: 0.5px; display: inline-block; line-height: 1; text-transform: uppercase;">${sub}</span>`;
        }
        return 'Quizy';
    }

    getLoggedInMenuHTML(isDashboard, isProfile) {
        return `
    ...
    async updateSubscriptionAndLogo(supabase, userId) {
        try {
            ...
            if (profile && profile.subscription) {
                ...
                if (logo) {
                    logo.innerHTML = this.getLogoHTML(profile.subscription);
    ...
    async connectedCallback() {
        ...
        if (isLogin) {
            ...
            initialMenuHTML = this.getLoggedInMenuHTML(isDashboard, isProfile);
        } else {
            initialMenuHTML = `
                ${this.getPwaInstallBtnHTML()}
        ...
        let logoHTML = this.getLogoHTML(hasSession ? cachedSub : null);
        let logoStyle = (hasSession && cachedSub && cachedSub !== 'none') ? 'style="display: inline-flex; align-items: center;"' : '';
        ...
        if (!isLogin) {
            if (user) {
                ...
                if (!currentHTML.includes('logoutBtn')) {
                    menu.innerHTML = this.getLoggedInMenuHTML(isDashboard, isProfile);
                ...
                await this.updateSubscriptionAndLogo(supabase, user.id);
            } else {
                localStorage.removeItem('quizy-subscription');
                menu.innerHTML = `
                    ${this.getPwaInstallBtnHTML()}
...
class QuizyBaseModal extends HTMLElement {
    constructor() {
        super();
        this.className = 'modal-overlay';
    }

    setupBaseListeners(cancelBtnId) {
        this.addEventListener('click', (e) => {
            if (e.target === this || (cancelBtnId && e.target.id === cancelBtnId)) {
                this.close();
            }
        });
    }
    ...
    renderModal({ maxWidth, headerStyle = '', bodyStyle = '', footerStyle = '', headerHtml, bodyHtml, footerHtml }) {
        this.innerHTML = `
            <div class="modal-card glass-panel" style="max-width: ${maxWidth};">
                <div class="modal-header" style="border-bottom: 1px solid rgba(255, 255, 255, 0.05); padding: 18px 24px; ${headerStyle}">
                    ${headerHtml}
                </div>
                <div class="modal-body" style="padding: 24px; ${bodyStyle}">
                    ${bodyHtml}
                </div>
                <div class="modal-footer" style="border-top: 1px solid rgba(255, 255, 255, 0.05); padding: 16px 24px 20px 24px; margin-top: 0; ${footerStyle}">
                    ${footerHtml}
                </div>
            </div>
        `;
...
class QuizyDeleteModal extends QuizyBaseModal {
    connectedCallback() {
        this.renderModal({
            maxWidth: '420px',
            headerHtml: '<h3 style="font-size: 1.3em; font-weight: 600; color: var(--text-light); margin: 0;">Set verwijderen</h3>',
            bodyHtml: `
        ...
        this.setupBaseListeners('btn-delete-cancel');
...
class QuizyWarningModal extends QuizyBaseModal {
    connectedCallback() {
        this.renderModal({
            maxWidth: '460px',
            headerStyle: 'display: flex; align-items: center; gap: 12px;',
            headerHtml: `
                <span class="material-symbols-rounded" style="color: #f59e0b; font-size: 26px;">warning</span>
                <h3 id="warning-modal-title" style="font-size: 1.2em; font-weight: 600; color: var(--text-light); margin: 0;"></h3>
            `,
        ...
        this.setupBaseListeners('btn-warning-cancel');
...
class QuizyRenameModal extends QuizyBaseModal {
    connectedCallback() {
        this.renderModal({
            maxWidth: '420px',
            headerHtml: '<h3 style="font-size: 1.3em; font-weight: 600; color: var(--text-light); margin: 0;">Mapnaam aanpassen</h3>',
            bodyStyle: 'display: flex; flex-direction: column; gap: 12px;',
            bodyHtml: `
        ...
        this.setupBaseListeners('btn-rename-cancel');
...
class QuizyConfirmModal extends QuizyBaseModal {
    connectedCallback() {
        this.renderModal({
            maxWidth: '420px; width: 90%',
            headerHtml: '<h3 id="confirm-modal-title" style="font-size: 1.2em; font-weight: 600; color: var(--text-light); margin: 0; text-align: left;">Sessie herstarten?</h3>',
            bodyStyle: 'display: flex; flex-direction: column; gap: 8px; text-align: left;',
            bodyHtml: `
            ...
            `,
            footerStyle: 'display: flex; justify-content: flex-end; gap: 12px; background: rgba(0,0,0,0.2);',
            footerHtml: `
        ...
        this.setupBaseListeners('btn-confirm-cancel');
